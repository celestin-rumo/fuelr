...
import type { WeightView } from "@app/lib/api";
...
it("logs a figure for a day, and nothing else", async () => {
  ...
  renderWithIntl(<WeightPanel weight={view()} today={TODAY} />);
...
it("draws nothing as a gap, and says so when there is nothing yet", () => {
  renderWithIntl(<WeightPanel weight={view({ entries: [], latest: null })} today={TODAY} />);
...
it("keeps only the form on the account page", () => {
  renderWithIntl(<WeightPanel weight={view()} today={TODAY} compact />);
  expect(screen.queryByTestId("weight-curve")).not.toBeInTheDocument();
