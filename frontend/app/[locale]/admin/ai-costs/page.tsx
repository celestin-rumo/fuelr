 ...
 * What the assisted reads have cost, and who made them.
 *
 * A section of the panel now rather than a page of its own — the role check
 * and the chrome come from the layout, and `/total-costs` redirects here for
 * whoever has the old address in a bookmark.
 */
export default async function AdminAiCostsPage() {
  const report = await aiCosts();
  if (!report) {
    notFound();
  }
