...
import { isCuisine } from "@app/lib/cuisines";
...
export default async function AppHomePage({
  ...
  const { q, tags, seasons, cuisines } = await searchParams;
  ...
  const fromThere =
    typeof cuisines === "string" ? cuisines.split(",").filter(isCuisine) : [];
  ...
  const filtering =
    term !== "" || selected.length > 0 || inSeason.length > 0 || fromThere.length > 0;
